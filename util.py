...
class EventData:
    ...
    def add_frame_data(self, data_folder, max_frames=100):
        timestamps = np.genfromtxt(join(data_folder, 'timestamps.txt'), max_rows=int(max_frames))
        frames = []
        frame_timestamps = []
        with open(join(data_folder, 'timestamps.txt')) as f:
            for line in f:
                fname, timestamp = line.split(' ')
                timestamp = float(timestamp)
                frame = cv2.imread(join(data_folder, 'frame_' + fname.zfill(10)+ '.png'), cv2.IMREAD_GRAYSCALE)
